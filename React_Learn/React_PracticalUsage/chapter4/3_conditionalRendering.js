...
// 변수가 배열일 경우, &&을 이용한 조건부 렌더링 코드 
// 변수가 배열일 경우에는 기본값으로 빈 배열을 넣어주는 것이 좋다.
// 다음 코드에서 students 배열의 기본값이 빈 배열이었다면, 조건부 렌더링을 할 때마다 students &&를 입력하지 않아도 되기 때문에 
// 편하게 map함수를 사용할 수 있다.
<div>{students && students.map(/* */)}</div>    // students가 undefined, null일 수 있다면 이렇게 작성해야 한다.
<div>{products.map(/* */)}</div>                // products배열의 기본값이 빈배열일 경우라면, 이렇게 작성해도 된다.
...
// 조건에 따라 아무것도 렌더링하지 않을 경우에는 null을 반환하기도 한다.
// 또는 해당 컴포넌트를 사용하는 부모 컴포넌트에서 조건에 따라 자식 컴포넌트를 렌더링(보이거나 가릴 수 있다.)할 수 있다.
// 단 이때는 해당 컴포넌트가 마운트와 언마운트를 반복할 수 있고, 이는 상탯값도 사라지고 렌더링 성능에도 안 좋은 영향을 끼칠 수 있다는 점을 알아두자.