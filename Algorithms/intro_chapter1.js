        ...
        4. 실수 비교하기 
            ...
            6. 그래서 비교할 실수의 크기들에 비례해 오차 한도를 정해야한다.p79
                ...
                3. 같다고 판단해야 할 경우 오차가 좀 있더라도 같다고 판단할 수 있는 만큼 수가 커야한다.
                    0. 두 수를 같은 수로 봐야 할 때 >> 오차범위 늘려서 두 수를 같은 수로 봐야함 >> 오차범위는 |a-b| 보다 항상 커야함
                    1. 같다고 판단해야 할 아주 큰 수 a, b 
                    2. 오차 한도 값이 항상 |a-b| 보다 커야한다. 이것 보다 크면 a,b는 같은 수라고 봐도 된다.
                    3. 따라서 |a-b|의 최대치를 구해내는게 관건이다.
                    4. 크게 나와야 하는 a는 최대한 크게나오는 것이 기준, 적게 나와야 하는 b의 최소가 기준이 된다.
                4. 다르다고 판단해야 할 경우 오차가 있더라도 다르다고 판단할 수 있는 만큼 수가 작아야 한다. 
                    0. 두 수를 다른 수로 봐야 할 때 >> 오차범위를 줄여서 다른 수로 봐야함 >> 오차범위는 |a-b| 보다 항상 작음
                    1. 다르다고 판단해야 할 아주 작은 수 a, b
                    2. 오차 한도 값이 항상 |a-b| 보다 작아야 한다. 이것 보다 작으면 두 a,b는 다른 수라고 봐도 된다.
                    3. 따라서 |a-b|의 최소치를 구해내는게 관건이다.
                    4. 적게나와야 하는 a는 최대한 적게 나오는 것이 기준이고, 많이 나와야하는 b는 최대한으로 많이 나와야 기준이 된다.
                5. 오차범위에 대해서
                    1. 오차범위는 어떤 오차범위안에서 나오는 수들은 다 같은 수로 본다는 의미의 오차범위이다.
                    2. 따라서 어떤 수를 다른 수로 보려면 오차범위를 최대한 작게해야 한다. 그래야 두 수가 겹치는 일이 없어 다른 수로 된다.
                    3. 그리고 그 오차범위는 절대적인 기준값이 있는게 아니라, 문제에 따라 내가 알아서 최대 최소를 결정해야한다.
                        0. 입력값의 최대/최소치를 구해야 한다. 문제에 따라 쉽게 구할수도있고 구할수없을수도있다.
                        1. 이렇게 스스로 범위를 정하려면 입력값이 오나전한 임의의 실수가 아니라, 일정한 단위를 가져야한다.
                        2. 문제 조건을 파악해서 구하려는 수의 최소나 최대 값을 도출한다.
                        3. 문제에서 소수점 둘째값 까지만 원한다. >> 단위가 미리단위 >> 즉 미리단위 이상 
                    4. 오차범위를 정해서 푸는건 많은 경험이 필요요한다. 그러니깐 상대 오차를 이용하자
            7. 상대 오차를 이용한다.
                1. 입력값이 한 자리가 주어질지 서른 자리가 주어질지 알 수 없을 때에는 오차 한도를 미리 정할 수 없다.
                2. 비교하는 값(a, b)들의 크기에 비례하여 오차를 정하는 방식을 선택해야 한다.
                3. realativeError(a,b) = |a - b| / max(|a| , |b|)
                4. 그러나 상대 오차 방법의 단점은 0과 너무 가까운 작은수를 서로 비교할 때 오류가 난다.
                5. 따라서 |a - b| 가 1e-10 보다 작으면 그냥 서로 같은 수라고 판단해야한다.
                6. |a - b| 가 1e-10 크면 상대오차 사용하여 두 실수가 같은지 다른지 알수 있다.
                    return Math.abs(a-b) <= 1e-8 * Math.max(Math.abs(a), Math.abs(b)) // true 면 같고 false면 다른 수
                    (1e-8 = 0.00000001) 



            8. 대소 비교 특히 등가 비교를 할때도 실수는 조심해야한다.
                1. a === b 여야 하는데 a가 약간의 오차로 다르게 나온다면 a > b or a < b로 나와서 a === b는 false가 된다.
                2. 이러한 문제를 피하기 위해선 두 수가 같은경우 즉 아주 가까운 경우의 수를 먼저 확인하고 로직을 처리해야한다.

            9. 실수라고 항상 정확하지 않은것은 아니다.
                1. 정확하게 표현할 수 있는 범위의 값은 항상 일정하고 정확하다. 
                2. 따라서 근사치가 적용되지 않는 범위한도내의 실수의 사칙연산은 항상 정확하고 변하지 않다.
                3. 따라서 위와 같은 조건일 경우에는 정수 변수같이 다뤄도 상관이 없다.
                4. 64비트의 가수부는 52비트이므로, 절대값이 +-2의52승 보다 작은 모든 정수들은 항상 정확하다.
                5. 같은 의미로(굳이 정수가 아니고 유리수라도) 2의 거듭제곱이 분모인 경우에는 정확하다.
                6. 추가적인 자료구조 혹은 임의 유리수 클래스를 만들어 분자와분모를 관리하는 경우에도 정확하다. 


            10. 코드의 수치적 안정성 파악하기
                1. 수치적으로 안정적인 코드에서는 실수의 정확도 문제를 고려하지 않아도 된다.
                2. 어떤 프로그램이 수치적으로 안정적이다라는 말은 실행하는 과정에 있어서 오차가 누적되지 않는 것을 의미한다.
                3. 코드 중간의 작은 오차가 프로그램 결과에서 그리 크게 부풀지 않았다면, 안정적인 코드라고 볼 수 있다.
                4. 알고리즘 문제들은 답에 작은 오차가 있어도 정답으로 인정한다.
                5. 그러나 이것도 엄청 어려운것이, 문제가 주어지는 입력값들에 따라 같은 알고리즈이라도 수치적으로 안정적일 수 있고
                그렇지 않을 수 있다. 이것은 많은 경험밖에 답이 없다. 

            11. NaN, 무한대 수가 나올경우에는 또 다른 지식이 또 필요하므로... 우선은 이것만 알아두자

            12. 가장 좋은 방법은 실수연산을 피하는 로직을 짜는 것이다.
                1. 곱셉과 나눗셈의 순서를 바꾸기 
                    1. 만약 어떤 세 정수를 계산하는데 그 결과가 실수가 아니고 반드시 정수가 나온다는 확신이 있다면
                    2. a / b * c >>> (a * c) / b 형태로 바꿔서 할 수 있다. 
                    3. 대신 위와같이 할 경우 오버플로우에 조심해야한다.
                2. 양변 제곱하기
                    1. 예를 들어 정수 좌표(x1, y1), (x2, y2) 두 점의 거리를 구해야 할 때,
                    2. Math.sqrt((x2-x1) + (y2-y1)) = r 를 이용하는게 아니라 양변에 제곱을해서
                    3. (x2-x1) + (y2-y1) = Math.pow(r,2) 를 사용하면 더 편하다.
                3. 실수 좌표를 써야하는 기하 문제에서는 좌표계를 가로 세로 모두 정수배로 늘리면 정수만으로 이용해 문제를 풀 수도 있다.
